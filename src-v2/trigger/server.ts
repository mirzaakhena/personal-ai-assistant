    ...
    req.on('end', () => {
      try {
        ...
        enqueue(phone_number, async () => {
          log.debug(`[TRIGGER] ${phone_number} — processing`);

          const sessionId = getSessionId(phone_number);
          const ctx: MessageContext = {
            sendMessage: (content: string) => gateway.sendMessage(phone_number, content),
            onAssistantMessage: (content: string) => trackAssistantMessage(phone_number, content),
          };
